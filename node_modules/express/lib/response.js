...
res.render = function(view, options, fn){
  ...
  if(result){
    options.data = result.getResult() ? result.getResult().toData() : {};
    options.error = result.getError();
    if(!result.isSuccess()){
        options.errorEvn = "alert(\""+options.error+"\");";
    }else {
        options.errorEvn = null;
    }
  }
  
  //默认往options中写入用户数据信息方便前端渲染
  if(req.session.baseUser){
    options.user = req.session.baseUser;
  }
